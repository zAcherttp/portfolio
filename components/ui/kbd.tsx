...
function KbdElement({ className, pressed = false, ...props }: KbdBaseProps) {
  return (
    <kbd
      className={cn(
        "inline-flex h-5 min-w-5 items-center justify-center rounded-sm border border-border bg-muted/60 px-1.5 align-middle font-mono text-[10px] font-medium leading-none text-muted-foreground shadow-[0_1px_0_var(--border)] transition-[transform,filter,box-shadow] duration-100 ease-out data-[state=pressed]:translate-y-px data-[state=pressed]:brightness-90 data-[state=pressed]:shadow-none",
        className,
      )}
      {...props}
